...
describe("Distributor", () => {
    ...
    describe("constructor", () => {
        ...
        it("does not accept 0x0 as RIP address", async () => {
            await expect(
                new Distributor__factory(owner).deploy(
                    treasuryFake.address,
                    ZERO_ADDRESS,
    ...
    describe("post-construction", () => {
        ...
        describe("distribute", () => {
            it("will do nothing if there are no recipients", async () => {
                ...
                // await expect(treasuryFake.mint()).to.have.callCount(0);
            });
            ...
            it("mint from treasury and distribute to recipients", async () => {
                ...
                ripFake.totalSupply.returns(10000000);
                await distributor.connect(staking).distribute();

                // await expect(treasuryFake.mint()).to.have.been.calledWith(staking.address, 29750);
                // await expect(treasuryFake.mint()).to.have.been.calledWith(other.address, 15210);
            });
        ...
        describe("nextRewardAt", () => {
            it("returns the number of RIP to be distributed in the next epoch", async () => {
                ripFake.totalSupply.returns(3899568500546135);
        ...
        describe("nextRewardFor", () => {
            it("returns the number of RIP to be distributed to the given address in the next epoch", async () => {
                const rate = 2975;
