...
describe("Bond Depository", async () => {
    ...
    // Initial mint for RIP and DAI (10,000,000)
    ...
    before(async () => {
        ...
        gRipFactory = await smock.mock("MockGRip");

        depositoryFactory = await ethers.getContractFactory("RipProtocolBondDepositoryV2");
    ...
    beforeEach(async () => {
        ...
        rip = await erc20Factory.deploy("RipProtocol", "RIP", 9);
        treasury = await smock.fake("ITreasury");
        gRIP = await gRipFactory.deploy("50000000000"); // Set index as 50
        staking = await smock.fake("RipProtocolStaking");
        ...
        await rip.mint(deployer.address, "10000000000000");
        await treasury.baseSupply.returns(await rip.totalSupply());

        // Mint enough gRIP to payout rewards
        await gRIP.mint(depository.address, "1000000000000000000000");

        await rip.connect(alice).approve(depository.address, LARGE_APPROVAL);
    ...
    it("should not redeem before vested", async () => {
        let balance = await rip.balanceOf(bob.address);
        let amount = "10000000000000000000000"; // 10,000
        await depository
            .connect(bob)
            .deposit(bid, amount, initialPrice, bob.address, carol.address);
        await depository.connect(bob).redeemAll(bob.address, true);
        expect(await rip.balanceOf(bob.address)).to.equal(balance);
    });
    ...
    it("should redeem after vested", async () => {
        ...
        const bobBalance = Number(await gRIP.balanceOf(bob.address));
        expect(bobBalance).to.greaterThanOrEqual(Number(await gRIP.balanceTo(expectedPayout)));
        expect(bobBalance).to.lessThan(Number(await gRIP.balanceTo(expectedPayout * 1.0001)));
    });

    it("should give correct rewards to referrer and dao", async () => {
        let daoBalance = await rip.balanceOf(deployer.address);
        let refBalance = await rip.balanceOf(carol.address);
        let amount = "10000000000000000000000"; // 10,000
        ...
        // Mint rip for depository to payout reward
        await rip.mint(depository.address, "1000000000000000000000");

        let daoExpected = Number(daoBalance) + Number((Number(payout) * daoReward) / 1e4);
        await depository.getReward();

        const frontendReward = Number(await rip.balanceOf(deployer.address));
        ...
        const carolReward = Number(await rip.balanceOf(carol.address));
