...
contract RipProtocolTreasury is RipProtocolAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    /**
     * @notice allow approved address to deposit an asset for RIP
    ...
    function deposit(
        ...
        // mint RIP needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        RIP.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn RIP for reserves
     * @param _amount uint256
     * @param _token address
     */
    ...
    /**
     * @notice mint new RIP using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        RIP.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or RIP from the treasury, using sRIP as collateral. This might allow an
     * sRIP holder to provide RIP liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(RIP)) {
            require(permissions[STATUS.RIPDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sRIP.changeDebt(value, msg.sender, true);
        require(sRIP.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(RIP)) {
            RIP.mint(msg.sender, value);
            ripDebt = ripDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sRIP.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with RIP
     * @param _amount uint256
     */
    function repayDebtWithRIP(uint256 _amount) external {
        require(
            permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.RIPDEBTOR][msg.sender],
            notApproved
        );
        RIP.burnFrom(msg.sender, _amount);
        sRIP.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        ripDebt = ripDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(RIP), _amount, _amount);
    }
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SRIP) {
            // 9
            sRIP = IsRIP(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(RIP.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns RIP valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(RIP)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return RIP.totalSupply() - ripDebt;
    }
}
