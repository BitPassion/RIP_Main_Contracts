...
import {IsRIP} from "../interfaces/IsRIP.sol";
import {SafeERC20} from "../libraries/SafeERC20.sol";
import {IYieldDirector} from "../interfaces/IYieldDirector.sol";
import {RipProtocolAccessControlled, IRipProtocolAuthority} from "../types/RipProtocolAccessControlled.sol";

/**
    @title YieldDirector (codename Tyche) 
    @notice This contract allows donors to deposit their sRIP and donate their rebases
            to any address. Donors will be able to withdraw their principal
            sRIP at any time. Donation recipients can also redeem accrued rebases at any time.
 */
contract YieldDirector is IYieldDirector, RipProtocolAccessControlled {
    ...
    struct DonationInfo {
        ...
        uint256 carry; // Amount of sRIP accumulated over on deposit/withdraw
    ...
    constructor(address sRip_, address authority_) RipProtocolAccessControlled(IRipProtocolAuthority(authority_)) {
        require(sRip_ != address(0), "Invalid address for sR.RIP");
    ...
    /**
        @notice Deposit sRIP, records sender address and assign rebases to recipient
        @param amount_ Amount of sRIP debt issued from donor to recipient
    ...
    */
    function deposit(uint256 amount_, address recipient_) external override {
        ...
        IERC20(sRIP).safeTransferFrom(msg.sender, address(this), amount_);

        uint256 index = IsRIP(sRIP).index();
    ...
    /**
        @notice Withdraw donor's sRIP from vault and subtracts debt from recipient
     */
    function withdraw(uint256 amount_, address recipient_) external override {
        ...
        IERC20(sRIP).safeTransfer(msg.sender, amount_);
    ...
    function withdrawAll() external override {
        ...
        uint256 sRipIndex = IsRIP(sRIP).index();
        uint256 total = 0;
        ...
        for (uint256 index = 0; index < donationsLength; index++) {
            ...
            recipient.agnosticDebt = _toAgnostic(recipient.totalDebt + recipient.carry);
            recipient.indexAtLastChange = sRipIndex;
        ...
        IERC20(sRIP).safeTransfer(msg.sender, total);

        emit AllWithdrawn(msg.sender, total);
    }

    /**
        @notice Get deposited sRIP amount for specific recipient
     */
    ...
    /**
        @notice Return total amount of donor's sRIP deposited
     */
    ...
    /**
        @notice Return total amount of sRIP donated to recipient since last full withdrawal
     */
    ...
    /**
        @notice Return total amount of sRIP donated from donor since last full withdrawal
     */
    ...
    /**
        @notice Get redeemable sRIP balance of a recipient address
     */
    ...
    /**
        @notice Redeem recipient's full donated amount of sRIP at current index
        @dev Note that a recipient redeeming their vault shares effectively pays back all
             sRIP debt to donors at the time of redeem. Any future incurred debt will
    ...
    function redeem() external override {
        ...
        recipient.carry = 0;
        recipient.indexAtLastChange = IsRIP(sRIP).index();

        IERC20(sRIP).safeTransfer(msg.sender, redeemable);
    ...
    /**
        @notice Get accumulated sRIP since last time agnostic value changed.
     */
    ...
    /**
        @notice Convert flat sRIP value to agnostic value at current index
        @dev Agnostic value earns rebases. Agnostic value is amount / rebase_index.
             1e9 is because sRIP has 9 decimals.
     */
    function _toAgnostic(uint256 amount_) internal view returns (uint256) {
        return (amount_ * 1e9) / (IsRIP(sRIP).index());
    }

    /**
        @notice Convert agnostic value at current index to flat sRIP value
        @dev Agnostic value earns rebases. Agnostic value is amount / rebase_index.
             1e9 is because sRIP has 9 decimals.
     */
    function _fromAgnostic(uint256 amount_) internal view returns (uint256) {
        return (amount_ * (IsRIP(sRIP).index())) / 1e9;
    }

    /**
        @notice Convert flat sRIP value to agnostic value at a given index value
        @dev Agnostic value earns rebases. Agnostic value is amount / rebase_index.
             1e9 is because sRIP has 9 decimals.
     */
