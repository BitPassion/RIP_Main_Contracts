...
contract sRipProtocol is IsRIP, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgRIP public gRIP; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked R.RIP", "sR.RIP", 9) ERC20Permit("Staked R.RIP") {
    ...
    function setgRIP(address _gRIP) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gRIP) == address(0), "gRIP:  gRIP already set");
        require(_gRIP != address(0), "gRIP:  gRIP is not a valid contract");
        gRIP = IgRIP(_gRIP);
    }
    ...
    /**
        @notice increases rRIP supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sRIP of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sRIP
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sRIP: insufficient balance");
    }
    ...
    // toG converts an sRIP balance to gRIP terms. gRIP is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gRIP.balanceTo(amount);
    }

    // fromG converts a gRIP balance to sRIP terms. sRIP is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gRIP.balanceFrom(amount);
    }

    // Staking contract holds excess sRIP
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gRIP.balanceFrom(IERC20(address(gRIP)).totalSupply())).add(
